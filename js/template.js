...
(function () {
  ...
  for (let i = 0; i < 80; i++)
    dots.push({ x: Math.random()*1400, y: Math.random()*1000, vx:(Math.random()-.5)*.2, vy:(Math.random()-.5)*.2, r:Math.random()*1.5+.3, a:Math.random() });
  (function frame() {
    ...
    dots.forEach(d => {
      ...
      ctx.fillStyle=`rgba(74,116,164,${d.a*.35})`;
      ctx.fill();
    });
...
function _applyAndRender() {
  ...
  switch(_curSort){
    ...
    case"free":    list.sort((a,b)=>Number(a.isPremium)-Number(b.isPremium)); break;
    case"premium": list.sort((a,b)=>Number(b.isPremium)-Number(a.isPremium)); break;
...
(async () => {
  ...
  document.getElementById('nav-auth-area').innerHTML = `
    <div style="position:relative" id="ubox">
      ...
      <div id="umenu" style="display:none;position:absolute;top:calc(100% + 10px);left:0;min-width:210px;background:var(--color-primary-dark);border:1px solid rgba(74,116,164,.18);border-radius:14px;padding:6px;box-shadow:0 16px 48px rgba(0,0,0,.5);z-index:9999;font-family:var(--font-main);">
        ...
        ${isAdmin?`<a href="../admin.html" style="display:flex;align-items:center;gap:9px;padding:9px 12px;border-radius:8px;color:rgba(242,242,242,.65);font-size:13px;font-weight:700;text-decoration:none;transition:background .14s;"
  ...
  document.addEventListener('click', e => {
    const box=document.getElementById('ubox'), menu=document.getElementById('umenu'), chev=document.getElementById('uchev');
    if(box&&!box.contains(e.target)&&menu){ menu.style.display='none'; if(chev) chev.style.transform='rotate(0)'; }
  });
})();
...
(function(){
  ...
  window.openCouponModal = function(item){
    ...
    document.getElementById('cmIcon').innerHTML = '<i class="fa-solid fa-lock"></i>';
    document.getElementById('cmIcon').style.cssText = 'background:rgba(74,116,164,.1);color:var(--accent)';
    document.getElementById('cmTitle').textContent = item.name || 'محتوى حصري';
    document.getElementById('cmSub').textContent = 'أدخل كوبونك للوصول لهذا القالب';
    const prev = document.getElementById('cmImgPreview');
    if(item.img){ prev.src=item.img; prev.style.display='block'; prev.style.background=item.darkBg?'#1a1a2e':''; }
    else prev.style.display='none';
    const buyS=document.getElementById('cmBuySection'), buyL=document.getElementById('cmBuyLink');
    if(item.promoLink){ buyS.style.display='block'; buyL.href=item.promoLink; }
  ...
  window.submitCoupon = async function(){
    ...
    if(code.length!==9){ hint.textContent='الكوبون مكوّن من 9 أرقام'; hint.className='cm-hint err'; inp.className='cm-input err'; inp.focus(); return; }
    const btn=document.getElementById('cmSubmitBtn'), spin=document.getElementById('cmSpin'), txt=document.getElementById('cmBtnTxt');
    btn.disabled=true; spin.style.display='block'; txt.textContent='...'; hint.textContent='';
    try{
      const {data:coupon,error}=await sb.from('coupons').select('id,is_used,used_by').eq('code',code).single();
      if(error||!coupon){ hint.textContent='الكوبون غير موجود ❌'; hint.className='cm-hint err'; inp.className='cm-input err'; return; }
      if(coupon.is_used){
        const {data:{session}}=await sb.auth.getSession();
        if(session&&coupon.used_by===session.user.id){
          hint.textContent='✓ كوبون مستخدم من قبلك — جاري الفتح...'; hint.className='cm-hint ok'; inp.className='cm-input ok';
          await _doUnlock(coupon.id,false,code);
        } else { hint.textContent='الكوبون مستخدم مسبقاً ❌'; hint.className='cm-hint err'; inp.className='cm-input err'; }
        return;
      }
      inp.className='cm-input ok'; hint.textContent='✓ كوبون صالح — جاري الفتح...'; hint.className='cm-hint ok';
      await _doUnlock(coupon.id,true,code);
    }catch(e){ hint.textContent='حدث خطأ، حاول مرة أخرى'; hint.className='cm-hint err'; console.error(e); }
    finally{ btn.disabled=false; spin.style.display='none'; txt.textContent='تفعيل'; }
  };
  ...
  async function _doUnlock(couponId,markUsed,code){
    try{
      ...
      if(markUsed){
        await sb.from('coupons').update({is_used:true,used_by:userId,used_at:new Date().toISOString(),image_type:_item.type}).eq('id',couponId);
        if(userId) await sb.from('activity_log').insert({action:'coupon_used',entity_type:_item.type,details:{coupon:code,item_id:_item.rawId,item_name:_item.name}}).catch(()=>{});
      }
      const _unlocked=JSON.parse(localStorage.getItem('sallim_unlocked')||'[]');
      if(_item.rawId&&!_unlocked.includes(_item.rawId)){_unlocked.push(_item.rawId);localStorage.setItem('sallim_unlocked',JSON.stringify(_unlocked));}
      sessionStorage.setItem('unlocked_item',JSON.stringify({type:_item.type,rawId:_item.rawId,name:_item.name,img:_item.img,darkBg:_item.darkBg,coupon:code}));
      document.getElementById('cmFormSection').style.display='none';
      document.getElementById('cmSuccessSection').style.display='block';
      document.getElementById('cmIcon').innerHTML='<i class="fa-solid fa-lock-open"></i>';
      document.getElementById('cmIcon').style.cssText='background:rgba(42,126,110,.1);color:#2a7e6e';
      setTimeout(()=>{
        const url=new URL('/editor.html',location.origin);
