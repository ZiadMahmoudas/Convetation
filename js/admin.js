   // ══ STATE ══
      ...
      async function loadOverview() {
        try {
          ...
          document.getElementById("s-cal").textContent =
            stats.total_calligraphy || 0;
          document.getElementById("s-bg").textContent =
            stats.total_backgrounds || 0;
          document.getElementById("s-cat").textContent =
        ...
        try {
          ...
          body.innerHTML =
            ...
            data
              .map(
                (r) =>
                  `<tr><td style="font-size:11px;color:rgba(247,242,232,.4)">${new Date(r.created_at).toLocaleString("ar-SA")}</td><td>${r.profiles?.username || "—"}</td><td><span class="badge badge-gold">${r.action}</span></td></tr>`,
              )
              .join("") +
            "</tbody></table>";
        } catch (e) {}
      ...
      async function saveCalligraphy() {
        ...
        const order =
          parseInt(document.getElementById("cal-order-inp").value) || 0;
        const tags = document
        ...
        if (!isEdit) {
          ...
          try {
            ...
            const editFile =
              document.getElementById("cal-files-edit")?.files?.[0];
            if (editFile) {
              const up = await Storage.upload(
                "calligraphy",
                editFile,
      ...
      async function saveBackground() {
        ...
        if (!isEdit) {
          ...
          try {
            ...
            const editFile =
              document.getElementById("bg-files-edit")?.files?.[0];
            if (editFile) {
              const up = await Storage.upload(
                "backgrounds",
                editFile,
      ...
      async function loadCoupons() {
        try {
          const { data: st } = await sb
            ...
            .single();
          if (st) {
            document.getElementById("cp-available").textContent =
              st.available ?? 0;
      ...
      function previewCouponCodes() {
        const n = Math.min(
          ...
          Math.max(
            1,
            parseInt(document.getElementById("cp-count").value) || 10,
          ),
        );
        const set = new Set();
        let tries = 0;
    ...
    : u.id !== currentAdminId 
      ...
      function filterUsers(q) {
        renderUsers(
          allUsers.filter(
            (u) =>
              (u.full_name || "").includes(q) || (u.username || "").includes(q),
          ),
        );
      }
        ...
        try {
          ...
          _featuredPicks = _allFeatured
            .map((f) => {
              ...
              return item
                ? { ...item, featId: f.id, sortOrder: f.sort_order }
                : null;
            })
            .filter(Boolean)
      ...
      function _renderFeatPool() {
        ...
        if (typeF && typeF !== "all")
          pool = pool.filter((p) => p.itemType === typeF);
        if (_featPoolSearch)
          pool = pool.filter((p) =>
